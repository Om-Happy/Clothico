...
public class JwtUtils {
	...
	public String generateJwtToken(Authentication authentication) {
		...
		CustomCustomerDetails customerPrincipal = (CustomCustomerDetails) authentication.getPrincipal();
//JWT : userName,issued at ,exp date,digital signature(does not typically contain password , can contain authorities
		return Jwts.builder() // JWTs : a Factory class , used to create JWT tokens
				.setSubject((customerPrincipal.getUsername())) // setting subject part of the token(typically user
																// name/email)
				...
				// project)
				.claim("user_id", customerPrincipal.getUser().getId())
	...
	public Authentication populateAuthenticationTokenFromJWT(String jwt) {
		...
		System.out.println("payloadClaims are :"+payloadClaims);
